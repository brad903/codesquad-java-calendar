...
public class BaseballGame {
	...
	// 예외처리2 : 중복값 입력했을 경우(피드백 반영)
	public static boolean duplicateException(int[] playerValue) {
		for (int i = 0; i < figures; i++) {
	...
	// 예외처리3 : 입력값에 0이 포함되었을 경우(피드백 반영)
	public static boolean zeroException(int[] playerValue) {
		for(int i=0; i<figures; i++) {
			if(playerValue[i] == 0) {
				return true;
			}
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			try {
				playerValue = valueParse(playerInput);  // 입력값 배열로 정리
			}
